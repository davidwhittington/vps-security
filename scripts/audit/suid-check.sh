...
#
# Usage:
#   bash scripts/audit/suid-check.sh              # compare against baseline
#   bash scripts/audit/suid-check.sh --update     # overwrite baseline with current state
#   bash scripts/audit/suid-check.sh --save       # alias for --update (first-run baseline)
set -uo pipefail
...
if [[ -n "$ADDED" ]]; then
    ...
    echo "    bash scripts/audit/suid-check.sh --update"
else
