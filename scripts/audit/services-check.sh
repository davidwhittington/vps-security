...
#
# Usage:
#   bash scripts/audit/services-check.sh              # compare against baseline
#   bash scripts/audit/services-check.sh --update     # overwrite baseline
set -uo pipefail
...
if [[ -n "$ADDED" ]]; then
    ...
    echo "    bash scripts/audit/services-check.sh --update"
else
