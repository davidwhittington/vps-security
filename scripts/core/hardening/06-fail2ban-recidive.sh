#!/usr/bin/env bash
# 06-fail2ban-recidive.sh — fail2ban recidive jail (permanent bans for repeat offenders)
#
...
for arg in "$@"; do
    ...
    if [[ "$arg" == "--help" || "$arg" == "-h" ]]; then
        echo "06-fail2ban-recidive.sh — configure fail2ban recidive jail for repeat offenders"
        echo
        echo "Usage:"
        echo "  bash scripts/core/hardening/06-fail2ban-recidive.sh [--dry-run] [--confirm]"
        echo
        echo "Flags:"
...
# linux-security: recidive jail — permanent bans for repeat offenders
