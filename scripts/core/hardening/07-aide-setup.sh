#!/usr/bin/env bash
# 07-aide-setup.sh — AIDE file integrity monitoring setup
#
...
#
# Usage:
#   bash scripts/core/hardening/15-aide-setup.sh
#   bash scripts/core/hardening/15-aide-setup.sh --dry-run
set -euo pipefail
...
for arg in "$@"; do
    ...
    if [[ "$arg" == "--help" || "$arg" == "-h" ]]; then
        echo "07-aide-setup.sh — install AIDE filesystem integrity monitoring with daily cron"
        echo
        echo "Usage:"
        echo "  bash scripts/core/hardening/07-aide-setup.sh [--dry-run] [--confirm]"
        echo
        echo "Flags:"
...
if ! $DRYRUN; then
    ...
    cat > "$AIDE_CONF" << 'EOF'
# linux-security AIDE configuration
...
# linux-security scripts installed to sbin
...
if $DRYRUN; then
    ...
    echo "# linux-security: weekly AIDE integrity check (Sunday 4AM)" > /etc/cron.d/aide-check
