...
for arg in "$@"; do
    ...
    if [[ "$arg" == "--help" || "$arg" == "-h" ]]; then
        echo "02-setup-admin-user.sh — create a non-root admin user with sudo and SSH key access"
        echo
        echo "Usage:"
        echo "  bash scripts/core/hardening/02-setup-admin-user.sh [--dry-run] [--confirm]"
        echo
        echo "Flags:"
