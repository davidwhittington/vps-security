...
for arg in "$@"; do
    ...
    if [[ "$arg" == "--help" || "$arg" == "-h" ]]; then
        echo "01-apache-hardening.sh — Apache security headers, CSP, ServerTokens hardening, and mod_status"
        echo
        echo "Usage:"
        echo "  bash scripts/web/hardening/01-apache-hardening.sh [--dry-run] [--confirm]"
        echo
        echo "Flags:"
