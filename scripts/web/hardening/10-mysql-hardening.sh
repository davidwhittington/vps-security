#!/usr/bin/env bash
# 10-mysql-hardening.sh — MySQL/MariaDB security baseline
#
...
#
# Usage:
#   bash scripts/web/hardening/17-mysql-hardening.sh
#   bash scripts/web/hardening/17-mysql-hardening.sh --dry-run
set -euo pipefail
...
for arg in "$@"; do
    ...
    if [[ "$arg" == "--help" || "$arg" == "-h" ]]; then
        echo "10-mysql-hardening.sh — run mysql_secure_installation equivalent and harden MySQL config"
        echo
        echo "Usage:"
        echo "  bash scripts/web/hardening/10-mysql-hardening.sh [--dry-run] [--confirm]"
        echo
        echo "Flags:"
...
if ! $DRYRUN; then
    cat > "$MYSQL_CONF" << 'EOF'
# linux-security MySQL/MariaDB security hardening
# Applied by scripts/web/hardening/17-mysql-hardening.sh
