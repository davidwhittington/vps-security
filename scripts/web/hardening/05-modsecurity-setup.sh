#!/usr/bin/env bash
# 05-modsecurity-setup.sh — ModSecurity with OWASP Core Rule Set
#
...
for arg in "$@"; do
    ...
    if [[ "$arg" == "--help" || "$arg" == "-h" ]]; then
        echo "05-modsecurity-setup.sh — install and configure ModSecurity WAF for Apache"
        echo
        echo "Usage:"
        echo "  bash scripts/web/hardening/05-modsecurity-setup.sh [--dry-run] [--confirm]"
        echo
        echo "Flags:"
...
# linux-security: OWASP CRS include — managed by vps-security
