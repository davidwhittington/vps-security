...
for arg in "$@"; do
    ...
    if [[ "$arg" == "--help" || "$arg" == "-h" ]]; then
        echo "02-log-monitoring-setup.sh — install logwatch and GoAccess for Apache log monitoring"
        echo
        echo "Usage:"
        echo "  bash scripts/web/hardening/02-log-monitoring-setup.sh [--dry-run] [--confirm]"
        echo
        echo "Flags:"
