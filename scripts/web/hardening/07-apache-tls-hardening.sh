#!/usr/bin/env bash
# 07-apache-tls-hardening.sh — Apache TLS cipher suite hardening
#
...
for arg in "$@"; do
    ...
    if [[ "$arg" == "--help" || "$arg" == "-h" ]]; then
        echo "07-apache-tls-hardening.sh — harden Apache TLS: disable old protocols and weak ciphers"
        echo
        echo "Usage:"
        echo "  bash scripts/web/hardening/07-apache-tls-hardening.sh [--dry-run] [--confirm]"
        echo
        echo "Flags:"
...
# linux-security: Apache TLS cipher suite hardening
